...
function OtpModal({ isModalOpen, setIsModalOpen, email, role }: OtpModalProps) {
  ...
  const handleResendOtp = async () => {
    try {
      ...
      toast({
        description: (
          <ErrorMessage message={getApiErrorMessage(error, Messages.RESEND_OTP_FAILED)} />
        ),
        className: "error",
      });
    }
  ...
  return (
    <AlertDialog open={isModalOpen}>
      <AlertDialogContent
        className="flex-col justify-items-center p-4 max-w-xl  max-h-1/2  
        ...
        <AlertDialogFooter>
          ...
          <div className="flex flex-col items-center gap-2">
            <AlertDialogAction
              ...
              className="bg-[#4B49AC] w-60 rounded-sm text-white hover:bg-[#3f3d91]"
            >
              Verify
            </AlertDialogAction>
            {timer === 0 && (
              <p className="text-sm">
                Didn&apos;t recieve code?{" "}
                <button className="text-[#4B49AC]" onClick={handleResendOtp}>
                  {" "}
                  Resend{" "}
                </button>
              </p>
            ...
            {timer !== 0 && (
              <p className="text-sm">
                Didn&apos;t recieve code?{" "}
                <button className="text-[#4B49AC]"> Resend in {timer} sec</button>
              </p>
            )}
          </div>
        </AlertDialogFooter>
