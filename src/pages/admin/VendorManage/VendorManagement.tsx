import { GetUserParams, getUsers, getVendors, updateUserStatus } from "@/api/admin/admin";
...
const VendorManagement = () => {
  ...
  const limit = 3;
  const [filterBlockedStatus, setFilterBlockedStatus] = useState<"all" | "blocked" | "unblocked">(
    "all"
  );
  ...
  const fetchVendors = async (page: number) => {
    ...
    try {
      ...
      const result = await getUsers(params, UserType.VENDOR);
      if (result) {
  ...
  const handleBlockToggle = async (id: string, action: string) => {
    try {
      ...
      await updateUserStatus(id, action, UserType.VENDOR);

      setVendors((prev) =>
        prev.map((sb) => (sb._id === id ? { ...sb, isBlocked: newStatus } : sb))
      );

      toast({
        description: (
          <SuccessMessage
            message={`User has been ${newStatus ? "blocked" : "unblocked"}.`}
            className=""
          />
        ),
      });
  ...
  const columns = useMemo(
    () => [
      { key: "name", label: "Name" },
      {
        key: "mobile",
        label: "Mobile",
        render: (row: Vendor) => <span className="text-[#8B5CF6]">{row.mobile}</span>,
      },
      {
        key: "email",
        label: "Email",
        render: (row: Vendor) => <span className="text-gray-300">{row.email}</span>,
      },
      { key: "estd", label: "Estd" },
      { key: "location", label: "Location", render: (row: Vendor) => row.location?.address },
      {
        ...
        render: (row: Vendor) => (
          <ConfirmDialog
            title={`Are you sure you want to ${row.isBlocked ? "unblock" : "block"} this Vendor?`}
            description="This action can be reversed later."
            confirmText={row.isBlocked ? "Unblock" : "Block"}
            variant="destructive"
            onConfirm={() => handleBlockToggle(row._id, row.isBlocked ? "unblock" : "block")}
            trigger={
              <Button size="sm" variant={row.isBlocked ? "constructive" : "destructive"}>
      ...
      },
      {
        ...
        label: "See More",
        render: (row: Vendor) => (
          <Ellipsis className="cursor-pointer" onClick={() => handleSingleUserPage(row)} />
        ),
      },
    ],
    [handleBlockToggle]
