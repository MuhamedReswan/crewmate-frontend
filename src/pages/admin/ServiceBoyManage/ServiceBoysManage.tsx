import { GetUserParams, getUsers, updateUserStatus } from "@/api/admin/admin";
...
const ServiceBoysManage = () => {
  ...
  const fetchServiceBoys = async (page: number) => {
    ...
    try {
      ...
      const result = await getUsers(params, UserType.SERVICE_BOY);
      if (result) {
  ...
  const handleBlockToggle = async (id: string, action: string) => {
    try {
      ...
      await updateUserStatus(id, action, UserType.SERVICE_BOY);
  ...
  const columns = useMemo(
    () => [
      { key: "name", label: "Name" },
      {
        key: "mobile",
        label: "Mobile",
        render: (row: ServiceBoy) => <span className="text-[#8B5CF6]">{row.mobile}</span>,
      },
      {
        key: "email",
        label: "Email",
        render: (row: ServiceBoy) => <span className="text-gray-300">{row.email}</span>,
      },
      { key: "age", label: "Age" },
